...
public class HurriedEggItem extends Item implements ProjectileItem {
    public HurriedEggItem(Properties properties) {
        super(properties);
    }
    ...
    public @NonNull InteractionResult use(Level level, Player player, @NonNull InteractionHand hand) {
        ...
        if (level instanceof ServerLevel serverLevel) {
            Projectile.spawnProjectileFromRotation(ThrownHurriedEgg::new, serverLevel, itemStack, player, 0.0F, 1.5F, 1.0F);
        }
    ...
    public @NonNull Projectile asProjectile(@NonNull Level level, Position position, @NonNull ItemStack itemStack, @NonNull Direction direction) {
        return new ThrownHurriedEgg(position.x(), position.y(), position.z(), level, itemStack);
    }
}
